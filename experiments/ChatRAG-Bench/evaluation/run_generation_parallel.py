...
from dataset import load_data, get_inputs_parallel, get_inputs_parallel_retrival
import torch
import os
import json
...
def get_prompt_list(args):
    ...
    return data_list
    #prompt_list = get_inputs_parallel(data_list, args.eval_dataset, tokenizer, num_ctx=args.num_ctx, max_output_len=args.out_seq_len)
...
def pred(rank, world_size, prompt_list, max_length, tokenizer, model, bos_token, args):
    ...
    output_list = []
    prompt_list = get_inputs_parallel_retrival(prompt_list, args.eval_dataset, tokenizer, num_ctx=args.num_ctx, max_output_len=args.out_seq_len)
    ...
    with torch.no_grad():
        for (instruction, documents, query) in tqdm(prompt_list):
            instruction = bos_token + instruction
            ...
            documents_lists = split_list_into_equal_parts(documents, 20)
